...
Options:
  ...
  -r, --random            Test with random sources generated by Csmith. Other test cases will be ignored. Frontend will be tested first, if it passes, test the backend.
...
CSMITH_GENERATED_FRONTEND_DIR = path.join(CSMITH_GENERATED_DIR, 'frontend')
CSMITH_GENERATED_BACKEND_DIR = path.join(CSMITH_GENERATED_DIR, 'backend')
...
def __choose_host_arglists(src: str, tmp_files: dict, frontend=False):
...
def test(src: str, tmp_files: dict, frontend=False):
  rel_path = path.relpath(src, TEST_DIR)

  input_text = __read_input(src)
  host_arglists = __choose_host_arglists(src, tmp_files, frontend=frontend)
  try:
...
def __init_random():
  global random_mode
  random_mode = True
  os.makedirs(CSMITH_GENERATED_DIR, exist_ok=True)
  os.makedirs(CSMITH_GENERATED_FRONTEND_DIR, exist_ok=True)
  os.makedirs(CSMITH_GENERATED_BACKEND_DIR, exist_ok=True)
...
def __test_src(src: str):
  ...
  if not random_mode:
    result = test(src, tmp_files, frontend=frontend_mode)
  else:
    result = test(src, tmp_files, frontend=True)
    if not result:  # frontend test passed
      shutil.copy(src, CSMITH_GENERATED_FRONTEND_DIR)
    else:
      result = test(src, tmp_files, frontend=False)
      if not result:  # test failed
        shutil.copy(src, CSMITH_GENERATED_BACKEND_DIR)
  for file in tmp_files.values():
    try:
      os.remove(file.name)
